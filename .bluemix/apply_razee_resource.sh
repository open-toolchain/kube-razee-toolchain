...
echo "=========================================================="
echo "CHECKING Razee Kapitan is installed in the cluster"
echo "Checking Razee Kapitan RemoteResource api-resource type registered:"
...
then
  echo "Razee Kapitan RemoteResource type already registered."
else
  echo "Razee Kapitan RemoteResource type does not yet exist."
  ...
  then
    red="\x1b[31m"
    no_color="\x1b[0m"
    echo -e "${red}Razee Kapitan not found in the cluster${no_color}"
  ...
  echo "Attempt register Razee Kapitan in cluster"
  echo "kubectl apply -f '${RAZEE_KAPITAN_URL}'"
  kubectl apply -f "${RAZEE_KAPITAN_URL}"
  echo "Verify Razee Kapitan resources exist"
  ...
  do
    ...
    then
      ...
      then
        ...
        echo "Found all needed Razee Kapitan resources.";
    ...
    echo -e "Attempt ${ITER} : Razee Kapitan resources not yet available. Rechecking shortly..."
    sleep 30
  done
  [[ $RAZEE_KAPITAN_STATUS == "OK" ]] || { echo "ERROR: Could not verify Razee Kapitan install succeeded ok, please check the log and try again."; exit 1; }
fi
